...
`qnm` の設計判断（特に **線形探索** と **安全策**）を固めるために、広く使われている代表的な実装を **8個**に絞って比較します。

- 対象: SciPy（BFGS / L-BFGS-B）, PyTorch（LBFGS）, libLBFGS（C）, LBFGSpp（C++）, Optim.jl（Julia）, JAXopt（JAX）, TensorFlow Probability（BFGS）
