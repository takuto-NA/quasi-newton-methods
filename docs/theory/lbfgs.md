...
Assume we store the most recent $m$ pairs $(s_i, y_i)$, with the oldest pair first and the newest pair last (this matches `qnm.lbfgs`).

```python
# python-like pseudocode
def two_loop_direction(g_k, s_history, y_history):
    q = g_k.copy()
    alpha = []
    rho = []

    # 1. Backward pass (newest -> oldest)
    # Apply the implicit product form to q
    ...
    # Use the most recent curvature to set the scale:
    # gamma_k = (s_last^T y_last) / (y_last^T y_last)
    if len(y_history) > 0:
        s_last = s_history[-1]
        y_last = y_history[-1]
        denom = dot(y_last, y_last)
        # Implementation note: if denom is extremely small, a practical guard is gamma_k=1.
        gamma_k = dot(s_last, y_last) / denom
    else:
        gamma_k = 1.0
    r = gamma_k * q

    # 3. Forward pass (oldest -> newest)
    # Undo the backward transforms in the correct order
    for s, y, alpha_i, rho_i in zip(s_history, y_history, reversed(alpha), reversed(rho)):
        beta = rho_i * dot(y, r)
        r = r + s * (alpha_i - beta)
...
Convergence theory for L-BFGS is more delicate than for full BFGS, so avoid reading this section as a blanket guarantee.
The safest high-level statement is:

- With a descent direction and a Wolfe/strong-Wolfe line search, classical arguments (e.g., Zoutendijk-type results in Nocedal & Wright, Chapter 3) give progress toward first-order stationarity.
- For L-BFGS specifically, additional assumptions (commonly phrased as boundedness of the eigenvalues of the implicit $H_k$ sequence, which is related to maintaining reliable curvature pairs) appear in standard analyses (see Nocedal & Wright, Chapter 7 and the original L-BFGS paper by Liu & Nocedal, 1989).

In practice, the key is to maintain the curvature condition $y^\top s > 0$ (or apply safeguards that reset/clear memory when it is violated), so that the computed direction remains a descent direction and line search behaves predictably.
